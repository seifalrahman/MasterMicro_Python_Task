from sympy import Symbol,Eq,solveset,solve,nsolve,lambdify,sympify, symbols, simplify, diff, integrate ,S, ConditionSet
...
def drawUserFunctions (exp1, exp2,Resolution):
    ...
    if flag==1 :
        ...
        x_vals=list(np.linspace(intersection_roots[0]-extension,intersection_roots[0],Resolution))
        ...
        for i in range(0,len(intersection_roots)-1):
            x_vals+=list(np.linspace(intersection_roots[i],intersection_roots[i + 1],Resolution))

        x_vals+=list(np.linspace(intersection_roots[len(intersection_roots)-1],intersection_roots[len(intersection_roots)-1]+extension,Resolution))
    elif len(intersection_roots)==1 :
        x_vals= list(np.linspace(intersection_roots[0]-20,intersection_roots[0]+20,Resolution))
    else:
        x_vals = list(np.linspace(-100, 100, Resolution))
